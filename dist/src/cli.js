...
async function main() {
    ...
    if (command === "smoke") {
        ...
        if (!project)
            throw new Error("weapp smoke requires --project or WEAPP_PROJECT");
...
`, client, project);
    ...
    if (command === "nodejs" || command === "run") {
        ...
        if (!code.trim())
            throw new Error("No JavaScript was provided");
        const client = await connectToDaemon(socketPath);
        try {
