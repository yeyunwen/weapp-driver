...
export class SessionManager {
    ...
    async use(projectInput, options, clientId, force = false) {
    ...
    require(projectInput, clientId) {
        const projectPath = resolveProjectPath(projectInput);
        const session = this.sessions.get(projectPath);
        if (!session)
            throw new Error(`Project session is not connected: ${projectPath}. Call useProject() first.`);
        if (session.ownership === "user")
            throw new OwnershipError(`The user currently controls project session ${projectPath}.`);
        if (session.activeClientId !== clientId)
            throw new SessionBusyError(`Client does not own active project session ${projectPath}.`);
        session.lastUsedAt = new Date();
        return session;
    }
    async list() {
    ...
    async complete(projectInput, clientId, keep) {
        const session = this.require(projectInput, clientId);
        if (keep) {
            session.activeClientId = null;
    ...
    async reset(projectInput, clientId) {
        const projectPath = resolveProjectPath(projectInput);
        const session = this.sessions.get(projectPath);
        if (!session)
            return { done: true, existed: false, projectPath };
        if (session.activeClientId && session.activeClientId !== clientId) {
            throw new SessionBusyError(`Project session is currently controlled by another agent process: ${projectPath}`);
        }
        await session.backend.close().catch(() => undefined);
...
async function assertDirectory(path) {
    ...
    if (!info?.isDirectory())
        throw new Error(`Mini Program project directory does not exist: ${path}`);
}
