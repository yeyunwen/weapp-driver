...
import type { ElementSummary, LayoutBox, SnapshotOptions, SnapshotResult } from "./types.js";
import { mapLimit } from "./util.js";

const DEFAULT_SELECTOR = "*";

export async function captureSemanticSnapshot(page: MiniPage, registry: RefRegistry, options: SnapshotOptions = {}): Promise<SnapshotResult> {
...
export async function captureSemanticElements(
  page: Pick<MiniPage, "path" | "query">,
  allElements: MiniElement[],
  registry: RefRegistry,
  options: SnapshotOptions = {},
  maxElements = options.maxElements ?? 300,
  ...
  if (allElements.length > maxElements) lines.push(`… truncated ${allElements.length - maxElements} elements at maxElements=${maxElements}`);
  return {
    content: lines.join("\n"),
    refs,
...
async function summarize(element: MiniElement, registry: RefRegistry, options: SnapshotOptions): Promise<ElementSummary> {
...
function formatElement(element: ElementSummary) {
  const parts = [element.ref, element.tag];
  if (element.text) parts.push(JSON.stringify(element.text));
  if (element.locator) parts.push(`[loc=${element.locator}]`);
  const attrs = Object.entries(element.attributes)
    .filter(([name]) => ["id", "class", "name", "type", "role", "data-testid", "data-qa", "aria-label"].includes(name))
    .map(([name, value]) => `${name}=${JSON.stringify(value)}`);
  if (attrs.length) parts.push(`[${attrs.join(" ")}]`);
  if (element.opaqueAttributes?.length) parts.push(`[opaque=${element.opaqueAttributes.join(",")}]`);
  if (element.box) parts.push(`{x:${element.box.x},y:${element.box.y},w:${element.box.width},h:${element.box.height}}`);
  return parts.join(" ");
}

function stableLocator(tag: string, attributes: Record<string, string>) {
  if (attributes.id) return `css:#${cssEscape(attributes.id)}`;
  if (attributes["data-testid"]) return `css:[data-testid=${JSON.stringify(attributes["data-testid"])}]`;
  if (attributes["data-qa"]) return `css:[data-qa=${JSON.stringify(attributes["data-qa"])}]`;
  if (attributes.name) return `css:${tag}[name=${JSON.stringify(attributes.name)}]`;
  return undefined;
}

function parseAttributes(wxml: string) {
...
function openingTag(wxml: string) {
...
async function readBox(element: MiniElement): Promise<LayoutBox | undefined> {
...
function numberFrom(value: Record<string, unknown>, keys: string[]) {
